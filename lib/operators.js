...
var expandList = require('./tools').expandList;
...
var operators = {
  ...
  replace: function(args) {
    ...
    if (argv.r) {
      this._find = new RegExp(argv._[0], 'g');
    } else {
      this._find = argv._[0];
    }
    this._replace = argv._[1];
  },
  ...
  outhtml: function(args) {
    ...
    this._stylefirstrow = true;
    if (!headerrow && (argv.H || argv['header-row'] === false)) {
      this._stylefirstrow = false;
    }
...
operators.cut.prototype._transform = function(chunk, encoding, done) {
  ...
  if (this._complement && !this._complemented) {
    this._cols = _.difference(_.range(json.row.length), this._cols);
    this._complemented = true;
  }

  _.each(this._cols, function(position) {
...
operators.grep.prototype._transform = function(chunk, encoding, done) {
  ...
  _.each(json.row, function(value, idx) {
    if (self._cols === undefined || self._cols.indexOf(idx) !== -1) {
...
operators.replace.prototype._transform = function(chunk, encoding, done) {
  ...
  var self = this;
  json.row = _.map(json.row, function(val, idx) {
    if (self._cols === undefined || self._cols.indexOf(idx) !== -1) {
      return val.replace(self._find, self._replace);
    } else {
      return val;
    }
  });
