...
operators.map.prototype._transform = function(chunk, encoding, done) {
  function sandbox_run() {
    sandbox.run(self._map_fn + "transform('" + chunk + "');", function(output) {
      var result = output.result.slice(1, -1);
      try {
        JSON.parse(result);
      } catch (e) {
  ...
  if (this._map_fn === undefined) {
    request(this._gistUri, function(error, response) {
      if (error !== null) {
        self.emit('error', new Error('Error opening gist.'));
        return;
      }
...
operators.map.prototype._flush = function(done) {
  function sandbox_run() {
    sandbox.run(self._map_fn + "flush();", function(output) {
      var result = output.result.slice(1, -1);
      try {
        JSON.parse(result);
      } catch (e) {
