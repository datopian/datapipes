...
var operators = {
  ...
  outcsv: function(args) {
    ...
    var argv = optimist()
      ...
      .parse(args)
    ;
    this._skipheaderrow = false;
    if (headerrow && (argv.H || argv['header-row'] === false)) {
      this._skipheaderrow = true;
      this._headerrowskipped = false;
    }
    if (argv.t) {
      // tabs switch overrides delimiter opt
      argv.d = '\t';
...
operators.outcsv.prototype._transform = function(chunk, encoding, done) {
  ...
  if (this._skipheaderrow && !this._headerrowskipped) {
    // we have a header row, we want to skip it,
    // we haven't skipped it yet
    this._headerrowskipped = true;
    return done();
  }
