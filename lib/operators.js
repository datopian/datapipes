...
var operators = {
  ...
  cut: function(args) {
    ...
    this._cols = ShorthandList(columns).expand();
  ...
  grep: function(args) {
    ...
    if (argv.c !== undefined) {
      this._cols = ShorthandList(argv.c).expand();
    }
...
operators.grep.prototype._transform = function(chunk, encoding, done) {
  ...
  var match = _.any(json.row, function(value, idx) {
    if (self._cols === undefined || self._cols.indexOf(idx) !== -1) {
      return self._pattern.test(value);
    }
    return false;
  });
